...
function fetchText(url, timeoutMs, redirectsLeft = 3) {
  return new Promise((resolve) => {
    ...
    const req = https.get(url, { timeout: timeoutMs, headers: { 'User-Agent': 'gpt-rundrive' } }, (res) => {
...
// A convenience mirror the pasted instruction reads at session start (under ~/.andymcp = a locked allowed root), so a remote AI can tell the user its instruction is stale. Never fatal — the console/panel banners stand alone.
