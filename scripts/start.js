...
if (updateInfo.mcp.updateAvailable) bar(`[update] Andy MCP Server ${updateInfo.mcp.current} → ${updateInfo.mcp.latest} — open the panel to pull & restart`);
if (updateInfo.rule.updateAvailable) bar(`[update] Rules ${updateInfo.rule.current} → ${updateInfo.rule.latest} — update in panel, then re-paste the Instructions (panel section 2) into the custom-instructions setting of each AI`);
